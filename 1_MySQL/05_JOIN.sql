...
SELECT * FROM employee; -- emp_id, emp_name, job_co
...
SELECT * FROM job; -- job_code, job_name
SELECT * FROM department; -- dept_id, dept title
...
SELECT * FROM dapartment; -- dept_id, dept_title, location_id
SELECT * FROM location; -- local_code, local_name, national_code
SELECT * FROM national; -- national_code, national_noam

SELECT emp_name, dept_title, local_name, national_name
FROM employee
     JOIN department ON
-- 5. 각 부서별 평균 급여를 조회하여 부서명, 평균 급여 조회
SELECT dapt_title, avg(slasry 
...
SELECT
       ...
       if(sal_lever IN('S1', 'S2'), '고급',
	      if(sal_lever IN('S3', 'S4'), '고급',

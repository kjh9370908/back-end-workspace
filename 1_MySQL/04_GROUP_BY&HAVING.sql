...
SELECT dept_code, count(bonus) -- 보너스가 null이 아닌 경우 
FROM employee
WHERE bonus IS NOT NULL
GROUP BY dept_code;

/*
   rollup|cube(컬럼1,컬럼2) (cube는 MySQL X) - 실제 볼일은 없는 집계 함수(SQLD 시험보면 나오긴함)
   ...
   grouping : rollup이나 cube에 의해 산출된 값이 해당 컬럼의 집함의 산출물이면 0, 아니면 1
   -> 집계된 값인지, 아닌지 정도만 구분 
   
   SQLD 자격증 시험에서 꼭 이상하게 나오기도 하지만... 실제 쓰이는 일은 없따! 
...
/*
   ...
   MINUS (차집함) : 선행 쿼리문의 결과값에서 후행 쿼리문의 결과값을 뺀 나머지 결과값만 추출 (My SQL X)
    --> INTERSECT, MINUS : WHERE절에서 AND 연산자를 사용해서 처리 가능!
*/
-- 1. UNION
-- (1) 부서 코드가 D5인 사원들의 사번, 사원명,부서코드, 급혀 조회
SELECT emp_id, emp_name, dept_code, salary
FROM employee
WHERE dept_code='D5'
UNION
-- (2) 급여가 300만원 초과인 사원들의 사번, 사원명, 부서코드, 
SELECT emp_id, emp_name, dept_code, salary
FROM employee

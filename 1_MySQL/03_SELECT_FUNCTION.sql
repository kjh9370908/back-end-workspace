...
/*
    ...
    LENGHT : 해당 문자열값의 BYTE 길이 수 반환
    -  한글 한 글자 -> 3BY
    -  영문자, 숫자, 특수문자 한글자-> 
    CHAR_LENGTH : 해당 문자열밗의 글자 수 변환 
*/
SELECT 
...
/*
	LPAD|PRAD(컬럼|'문자열', 최종적으로 반환할 문자의 길이, '덧붙이고자 하는 문자')
...
*/
SELECT trim('_______K H_________'); -- 기본적으로 양쪽에 있는 공백 제거
SELECT trim(BOTH '' FROM'_____________K H_____________');

SELECT trim(LEADING 'Z' FROM 'ZZZKHZZZ'); -- KHZZZ <--- 문자 제거는 LEADING
SELECT ltrim('_______K H_________'); -- LTRIM : 앞쪽 공백만 제거 
...
SELECT  emp_name, email "변경 전"
Replace(email

-- 2. 사원명과 주민등록번호 (000000-0000000)으로 조회
-- replace, rpad, conca
SELECT emp_name, REPLACE(emp_no, substr(emp_no -6, -6), "******")
FROM empolyee;

SELECT emp_name, rpad(substr(emp_no, -6, 6), "******")
FROM employee;
-- rpad
SELECT empp_name, rpad(substr(emp_no, 1, 8), char_lenghrh(emp_no), "*")
FROM employee;
...
/*
     ...
     - 반올림한 결과를 반환 
*/
SELECT round(123.456), round(123.567, 2), round(123.567, -1);
...
*/
SELECT 
now(), adddate(now(), interval 10 year),
subdate(now(), interval 15day ),
...
SELECT emp_name, date_format(hire_date, '%2024.%06.%19.%10.%1')
FROM employee;
...
-- 전 사원의 적원명, 보너스, 보너스 포함 연봉,(급여 + 급여 * 보너스) * 12 조회 
SELECT emp_name, bonus, (salary + salary * ifnull(bonus, 0) * 12)
FROM employee;

-- 직원명, 부서코드(dept_code) 조회 ( 부서코드가 없으면 '부서없음)
SELECT employee, ifnull(dept_code, '부서없음')
FROM employee;

/*
  NULLIF(값1, 값2) 
  - 두 개의 값이 동일하면 null 반환, 두 개의 값이 동일하지 않으면 값 1 반환 
*/
SELECT nillif('123','123'), nullif('123', 456) -- null, 123

/*
   --IF(값1, 값2, 값3)  | IF(조건, 조건이 True인 경우, 조건이 False인 경우)
    - 값1이null이 아니면 값2 반환, null이면 값3반환 
    - 조건에 해당하면 두번째 값 반환, 해당하지 않으면 마지막 값 반환
    
*/
SELECT emp_name, bonus, if(bonusm 0.7, 0.1)
FROM employee;
...
SELECT 
emp_id, emp_name, emp_no, if(substr(emp_no,8, 1) = 1, '남', '여' ) 성별 
FROM emloyee;
...
/*
   ...
   - 모든 그룹 함수는 NULL 값을 자동으로 제외하기 때문에
     AVG 함수를 사용할 때는 COALESCE 또는 IFNULL 함수와 함께 하용하는 걸 권장
*/

/*
     ...
     CRUD: Create(추가) Read(보기) Update(수정) Delete(삭제)
     - 관계형(Relational) 데이터베이스
     1. 가장 대표적인 데이터베이스 시스템
     2. 데이터를 테이블 형태로 저장하고 여러 테이블을 조합하여 비즈니스 관계를 도출하는 구조
     3. 데이터의 중복을 최소화할 수 있으며 사용하기 편리하고 데이터의 무결성, 트랜잭션 처리 등 데이터베이스 관리
     시스템으로 뛰어난 성능을 보여준다.
	 ...
	 2. DQL(Data Query Language) : 데이터 질의어
            -데이터를 조회(검색)하기 위해 사용하는 언어(SLECT : 조회)
     3. DLC(Data Control Language) : 데이터 제어어 
	        -DBMS에 대한 보안, 무결성, 복구 등 DBMS를 제어하기 위한 언어
            (GRANT : 권한 부여, REVOKE : 권한 회수)
	 4. DLL(Data Definition Language) : 데이터 정의어
...
/*
 ...
 FROM: 테이블명: 
       ...
       -SELECT를 통해서 조회된 결과를 RESULT SET이라고 한다. (즉, 조회된 행동의 집헙)
       - 조죄하고자 하는 컬럼들은 반드시 FROM 절에 기술한 테이블에 존제하느 컬럼이어야 한다.
...
-- 전화번호(phoen), 입사일(hire_date) 정보만 조회
SELECT emp_name, email, phoen, hire_date
FROM employee;
...
-- 급여(salary) 조회 
  ...
  /*
    컬럼 산술 연산 
    -SELECT 절에 컬럼명 입력 부분에 산술연산자를 사용하여 결과를 조죄할 수 있다. 
  */
  -- employee 테이블에서 직원명(emp_name), 직원 영봉(salary * 12) 조회 
...
emp_name 직원명,
  ...
  */
  ...
  -- 사원들의 사원명(emp_name), 사번(emp_id), 급여(salary) 조회  
  SELECT emp_name, emd_id, salary
  FROM employee
  ...
  /*
     ...
     비교대상컬럼 LIkE '_문자' => 비교대상컬럼값에 문자앞에 무조건 한글자가 올 경우 조회
     비교대상컬럼 LIkE '_문자' => 비교대상컬럼값에 문자 앞과 뒤에 무조건 한글자씩 올 경우 
  */
  -- 사원들 중 성이 전씨인 사원들의
  ...
  -- 이름 중에 '하'가 포함된 사원들의 사원명, 주민번호(emp_no), 전화번호(phoen) 조회 
  SELECT emp_no, phoen
  FROM employee
  WHERE emp_name LIKE '%하%';
  
  
  -- 이름이 가운데 글자가 '하'인 사원들의 사원명, 전화번호 조회 (이름이3글자)
  SELECT emp_name, phoen
  FROM employee
  ...
  SELECT emp_id, emp_name, phoen, email
  FROM employee
  WHERE phoen LIKE '__1%';
...
-- 부서배치를 아직 받지 않고 보너스를 받은 사원들의
-- 이름, 보너스, 부서코드(dept_code) 조회
SELECT emp_name, bonusm, dept_code
FROM employee
...
--    사수비번(manager), 부서코드(dept_code) 조회
...
SELECT emp_id, emp_name, salary, bouns
FROM employee
WHERE salary * 12 >= 40000000 AND bouns IS NULL;
...
SELECT emp_id, emp_name, salary, hire_date, bouns
FROM employee
...
SELECT emp_id, emp_name, salary, (salary + salary * bouns) * 12 연봉 
FROM employee
WHERE (salary + salary * bouns) * 12 IS NOT NULL
     AND emp_name LIKE '%하%'
ORDER BY 연봉 DESC;

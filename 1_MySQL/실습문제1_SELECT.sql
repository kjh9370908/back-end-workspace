...
SELECT substr(address, instr(address, ' ') + 1)address, adistrict ''
FROM address 
WHERE district != ''
ORDER BY 2, 1 DESC
LIMIT 0,10;
...
SELECT 
      ...
      char_length(concat(first_name, last name)) 글자수 
FROM actor
...
SELECT distinct substr(descriptrion, 1 , instr(description, 'of')-2) "of 이전 문장"
FROM film
ORDER BY 1 DESC
LIMIT 10;
-- 9. film 테이블에서 replacement_cost 최소 비용과 최대 비용 조회
SELECT min(replacement_cost) "최소 비용", max(replace 